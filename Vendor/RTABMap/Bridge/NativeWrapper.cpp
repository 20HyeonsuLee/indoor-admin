...
// Sprint 35 Cycle 2 — Step A: ULogger 레벨/타입 설정 + stderr line-buffering
// RTAB-Map 내부 LOGI/LOGW/UERROR 등은 ULogger 싱글턴을 통해 출력됨.
// kTypeConsole = stderr 출력. iOS device에서 Xcode Device Logs 탭에서 확인 가능.
// NSLog 리다이렉트는 Swift 측 RTABMapBridge.start()에서 수행.
#include <unistd.h>
#include <stdio.h>

static bool s_logRedirectStarted = false;

static void startNativeLogRedirect()
{
    if (s_logRedirectStarted) return;
    s_logRedirectStarted = true;

    // ULogger: kTypeConsole → stderr 출력, Info 레벨 이상 전부 출력
    ULogger::setType(ULogger::kTypeConsole);
    ULogger::setLevel(ULogger::kInfo);

    // stderr를 line buffering으로 — 버퍼 쌓임 없이 줄 단위 즉시 플러시
...
{
    // Step A: native stderr → NSLog redirect를 앱 생애 최초 1회 시작
    startNativeLogRedirect();
